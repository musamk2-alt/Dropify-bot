...
const COMMANDS = {
  ...
  // PERSONAL VIEWER DISCOUNT -> backend /viewer
  discount: {
    ...
    execute: async (channel, tags) => {
      ...
      try {
        const data = await callBackend(
          `/api/discounts/${twitchLogin}/viewer`,
          "POST",
          { viewerName: username }
        );

        if (!data.ok) {
          console.error("Viewer discount backend error:", data);
          return client.say(
            channel,
            `@${username} something went wrong. The streamer might not have Shopify connected yet.`
          );
        }

        const code = data.discount.code;
